...
const loadSettings: BackgroundEffect = (store) => async (action) => {
	if (action.type === BackgroundActionType.SETTINGS_LOAD) {
		...
		const state: SettingsState = {
			...
			sites,
			permissions,
			wordReplacementEnabled: settings.wordReplacementEnabled || false,
			customWordReplacements: settings.customWordReplacements || [],
			displayMode: settings.displayMode || 'quotes',
		};
