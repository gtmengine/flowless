...
const QuoteOptions = (store: Store) => {
	...
	const displayModeSelector = h('div.v-stack-2', [
		h('label', [
			h('span', 'Display Mode:'),
		]),
		h('div.h-stack-2', [
			h('label', [
				h('input', {
					props: {
						type: 'radio',
						name: 'displayMode',
						checked: state.settings.displayMode === 'quotes',
					},
					on: {
			...
			h('label', [
				h('input', {
					props: {
						type: 'radio',
						name: 'displayMode',
						checked: state.settings.displayMode === 'blank',
					},
					on: {
						change: () => store.dispatch({
							type: ActionType.BACKGROUND_ACTION,
				...
				h('span', ' Blank (Completely Hidden)'),
			]),
			h('label', [
				h('input', {
					props: {
						type: 'radio',
						name: 'displayMode',
						checked: state.settings.displayMode === 'cats',
					},
					on: {
						change: () => store.dispatch({
							type: ActionType.BACKGROUND_ACTION,
